...
#[allow(unused)]
const FN: [f64; 128] = [
    1.0, 0.9635997, 0.9362827, 0.9130436, 0.89228165, 0.87324303,
...
impl NormalDistribution {
    #[allow(unused)]
    fn new(sd: i64, mean: f64, std_dev: f64) -> Option<NormalDistribution> {
        Some(NormalDistribution{
            mean,
            std_dev,
            source: RngSource::new(sd),
        })
    }
    ...
    fn rng(&mut self) -> f64 {
        loop {
            let j: u32 = self.source.rng();
            let i = (j & 0x7f) as usize;
            let (s, jj) = ((j & 0x80000000u32) != 0, j & 0x7fffffff);
            let j = if s {-(jj as i32)} else {jj as i32};
            let mut x = f64::from(j) * (WN[i] as f64);
            if jj < KN[i] {
                return x * self.std_dev + self.mean;
            } else {
                if i == 0 {
                    loop {
                        let r:f64 = self.source.rng();
                        x = r.ln() * (1.0f64 / RN);
                        let y: f64 = self.source.rng();
                        let y = y.ln();
                        if y+y > x*x {
                            break;
                        }
                    ...
                    if j > 0{
                        return (RN + x) * self.std_dev + self.mean;
                    }
                    return (-RN - x) * self.std_dev + self.mean;
                } 
                let r: f64 = self.source.rng();
                if FN[i] + r * (FN[i-1] - FN[i]) < (-0.5 * x * x).exp() {
                    return x * self.std_dev + self.mean;
                }
            }
        }
...
#[cfg(test)]
mod tests {
    use crate::math::rand::normal_distribution::NormalDistribution;

    #[test]
    fn test_normal_distribution() {
        let r = NormalDistribution::new(1,0f64,1f64);
        assert!(r.is_some());
        let mut r = r.unwrap();
        let mut v = [0f64;100];

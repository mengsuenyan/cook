...
impl BigFloat {
    ...
    fn align_exp(&self, rhs: &BigFloat) -> (Nat, Nat, i32) {
        if self.exp <= rhs.exp {
            ...
            (&self.mantissa << (e as usize), rhs.mantissa.clone(), rhs.exp)
        } else {
            let e = self.exp - rhs.exp;
            (self.mantissa.clone(), &rhs.mantissa << (e as usize), self.exp)
        }
    }
...
bf_from_int_macro!(u8, |ele| -> (Nat, Form) {(Nat::from_u8(ele), Form::Finite)});
bf_from_int_macro!(u16, |ele| -> (Nat, Form) {(Nat::from_u16(ele), Form::Finite)});
bf_from_int_macro!(u32, |ele| -> (Nat, Form) {(Nat::from_u32(ele), Form::Finite)});
bf_from_int_macro!(usize, |ele| -> (Nat, Form) {(Nat::from_usize(ele), Form::Finite)});
bf_from_int_macro!(u64, |ele| -> (Nat, Form) {(Nat::from_u64(ele), Form::Finite)});
bf_from_int_macro!(u128, |ele| -> (Nat, Form) {(Nat::from_u128(ele), Form::Finite)});
bf_from_int_macro!(i8, |ele: i8| -> (Nat, Form) {(Nat::from_u8(ele.abs() as u8), Form::Finite)});
bf_from_int_macro!(i16, |ele: i16| -> (Nat, Form) {(Nat::from_u16(ele.abs() as u16), Form::Finite)});
bf_from_int_macro!(i32, |ele: i32| -> (Nat, Form) {(Nat::from_u32(ele.abs() as u32), Form::Finite)});
bf_from_int_macro!(isize, |ele: isize| -> (Nat, Form) {(Nat::from_usize(ele.abs() as usize), Form::Finite)});
bf_from_int_macro!(i64, |ele: i64| -> (Nat, Form) {(Nat::from_u64(ele.abs() as u64), Form::Finite)});
bf_from_int_macro!(i128, |ele: i128| -> (Nat, Form) {(Nat::from_u128(ele.abs() as u128), Form::Finite)});
...
bi_from_float_macro!(f32, |val: f32, is_normal: bool, vb: u32| -> (Nat, i32) {
    ...
    let e = if is_normal { (((v[3] << 1) | (v[2] >> 7)) as i32) - 127 } else { -126 };
...
bi_from_float_macro!(f64, |val: f64, is_normal: bool, vb: u32| -> (Nat, i32) {
    let v = val.to_le_bytes();
    let tail = (v[0] as u64) + ((v[1] as u64) << 8) + ((v[2] as u64) << 16) +
        ((v[3] as u64) << 24) + ((v[4] as u64) << 32) + ((v[5] as u64) << 40) + 
        (((v[6] & 0xf) as u64) << 48);
    let e = if is_normal { (((v[6] >> 4) as i32) | (((v[7] & 0x7f) as i32) << 4)) - 1023} else {-1022};

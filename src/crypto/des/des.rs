...
use crate::crypto::des::const_table as mct;
...
impl DesCipher {
    pub fn new(key: [u8; 8]) -> DesCipher {
        DesCipher {
            ks: DesCipher::key_schedule(key),
        }
    }

    #[inline]
    fn cvt_slice_to_u64(src: &[u8]) -> u64 {
        super::super::super::cfg_if! {
    ...
    fn crypt_block(&self, dst: &mut [u8; 8], src: &[u8], is_encrypt: bool){
        let data = DesCipher::permute(DesCipher::cvt_slice_to_u64(src), mct::DES_IP.as_ref());
        let (mut l_pre, mut r_pre) = (data & (u32::max_value() as u64), data >> 32);
        
        if is_encrypt {
            for i in 0..16 {
                let tmp = r_pre;
                r_pre = l_pre ^ DesCipher::feistel(r_pre, self.ks[i]);
                l_pre = tmp;
            }
        } else {
            for i in 0..16 {
                let tmp = r_pre;
                r_pre = l_pre ^ DesCipher::feistel(r_pre, self.ks[15 - i]);
                l_pre = tmp;
            }
        }
        
        let pre_output = (l_pre << 32) | r_pre;
        *dst = DesCipher::cvt_to_bytes(DesCipher::permute(pre_output, mct::DES_IIP.as_ref()));
    }
    
    /// 输入32位的R和48位的k, 输出32位  
    fn feistel(r: u64, k: u64) -> u64 {
        let r_p = DesCipher::f_expand(r);
        let t = r_p ^ k;
        let s_p = DesCipher::f_sbox(t);
        DesCipher::permute(s_p, mct::DES_P.as_ref())
    }
    
    /// in: 32 out: 48  
    fn f_expand(r: u64) -> u64 {
        DesCipher::permute(r, mct::DES_E.as_ref())
    }
    ...
    fn f_sbox(t: u64) -> u64 {
        let mut output = 0u64;
        let e = DesCipher::cvt_to_bytes(t);
        ...
        let b8 = e[5] & 63;
        output |= ((mct::DES_S[0][DesCipher::sbox_idx(b1)] as u64) << 4) + (mct::DES_S[1][DesCipher::sbox_idx(b2)] as u64);
        output |= (((mct::DES_S[2][DesCipher::sbox_idx(b3)] as u64) << 4) + (mct::DES_S[3][DesCipher::sbox_idx(b4)] as u64)) << 8;
        output |= (((mct::DES_S[4][DesCipher::sbox_idx(b5)] as u64) << 4) + (mct::DES_S[5][DesCipher::sbox_idx(b6)] as u64)) << 16;
        output |= (((mct::DES_S[6][DesCipher::sbox_idx(b7)] as u64) << 4) + (mct::DES_S[7][DesCipher::sbox_idx(b8)] as u64)) << 24;
    ...
    fn key_schedule(key: [u8; 8]) -> [u64; 16] {
        ...
        let key = DesCipher::cvt_from_bytes(key);
        let mut k_pre = DesCipher::permute(key, mct::DES_PC1.as_ref());
        for i in 0..ROWS {
            let tmp = DesCipher::ks_rotate(k_pre, mct::DES_LS[i]);
            output[i] = DesCipher::permute(tmp, mct::DES_PC2.as_ref());
            k_pre = tmp;
        }
    ...
    fn permute(key: u64, permutation: &[u8]) -> u64 {
        let mut k_p = 0u64;
        for (i, &ele) in permutation.iter().enumerate() {
            let b = (key >> (ele as u64)) & 0x1;
            k_p |= b << (i as u64);
        }
        k_p
    }
    ...
    fn ks_rotate(key: u64, cl: u8) -> u64 {
        let (sl, sr) = (cl, 8 - cl);
        let v = DesCipher::cvt_to_bytes(key);
        let mut output = 0;
...
mod tests {
    ...
    fn des() {
        ...
        for ele in cases.iter() {
            ...
            cipher.decrypt(&mut decrypt, encrypt.as_slice());
